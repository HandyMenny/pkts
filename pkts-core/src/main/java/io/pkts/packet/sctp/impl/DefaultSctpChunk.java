...
public class DefaultSctpChunk implements SctpChunk {
    ...
    public static DefaultSctpChunk frame(final Buffer buffer) {
        try {
            ...
            assertArgument(
                    buffer.getReadableBytes() >= 4, "There must be at least 4 bytes to read");
            ...
            assertArgument(
                    buffer.getReadableBytes() >= length + padding,
                    "Unable to read Chunk Value. Not enough bytes. Needed "
                            + (length + padding)
                            + " bytes but only "
                            + buffer.getReadableBytes()
                            + " bytes available");
    ...
    private DefaultSctpChunk(
            final Type type, final Buffer header, final Buffer valueOnWire, final int padding) {
        this.type = type;
        this.header = header;
     ...
     * value. Also, it does not include padding.
     */
    @Override
