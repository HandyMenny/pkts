    ...
    public IPv6Packet frame(final MACPacket parent, final Buffer payload) throws IOException {
        ...
        if (version != IPv6Packet.VERSION_IDENTIFIER) {
            throw new FramingException(
                    String.format("Invalid IPv6 version: %d", version), Protocol.IPv6);
        }
        ...
        final int totalLength =
                IPv6PacketImpl.FIXED_HEADER_LENGTH
                        + extensionHeadersBuffer.getWriterIndex()
                        + payloadLength;
        final Buffer data = payload.slice(Math.min(totalLength, payload.capacity()));
        return new IPv6PacketImpl(
                parent, Buffers.wrap(fixedHeader, extensionHeadersBuffer), nextHeader, data);
    }

    private int accumulateNextHeader(
            final int protocolNumber, final Buffer payload, final Buffer extensionHeadersBuffer)
        ...
        switch (protocolNumber) {
            ...
            case IPv6Packet.EXTENSION_DESTINATION_OPTIONS:
                ...
                headerExtensionLen =
                        8 + payload.getByte(IPv6PacketImpl.FIXED_HEADER_LENGTH + 1) * 8;
                break;
            ...
            case IPv6Packet.EXTENSION_AH:
                nextHeaderProtocol.set(payload.getByte(IPv6PacketImpl.FIXED_HEADER_LENGTH));
                headerExtensionLen =
                        4 * (payload.getByte(IPv6PacketImpl.FIXED_HEADER_LENGTH + 1) + 2);
                break;
            case IPv6Packet.EXTENSION_ESP:
                // TODO figure out how length is even parsed...
                // If we can't parse the header, we have to throw, because we need to parse it to
                // find the next header.
            default:
                throw new FramingException(
                        String.format("Unsupported IPv6 extension header: %d", protocolNumber),
                        Protocol.IPv6);
        }
