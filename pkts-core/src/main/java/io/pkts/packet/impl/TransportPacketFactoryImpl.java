...
public final class TransportPacketFactoryImpl implements TransportPacketFactory {
    ...
    /**
     * Raw Ethernet II frame with a source and destination mac address of 00:00:00:00:00:00 and the
     * type is set to IP (0800 - the last two bytes).
     */
    private static final byte[] ehternetII =
            new byte[] {
                (byte) 0x00,
                (byte) 0x00,
                (byte) 0x00,
    ...
    /**
     * Raw IPv4 frame with source and destination IP:s set to 127.0.0.1 and a protocol for UDP. The
     * length and checksums must be corrected when generating a new packet based on this template.
     */
    private static final byte[] ipv4 =
            new byte[] {
                (byte) 0x45, (byte) 0x00, (byte) 0x01, (byte) 0xed, (byte) 0x00, (byte) 0x00,
                        (byte) 0x00, (byte) 0x00,
                (byte) 0x40, (byte) 0x11, (byte) 0x3a, (byte) 0xfe, (byte) 0x7f, (byte) 0x00,
                        (byte) 0x00, (byte) 0x01,
                (byte) 0x7f, (byte) 0x00, (byte) 0x00, (byte) 0x01
            };

    /**
     * Raw UDP frame where the source port is 5090 and the destination port 5060. You will certainly
     * have to change the length of the UDP frame based on your payload but you also need to
     * re-calculate the checksum. And you probably want to
     */
    ...
    public TransportPacket create(
            final Protocol protocol,
            final byte[] srcAddress,
            final int srcPort,
            final byte[] destAddress,
            final int destPort,
            final Buffer payload)
            throws IllegalArgumentException, IllegalProtocolException {
        final TransportPacket pkt = createUdpInternal(payload);
    ...
    @Override
    public UDPPacket createUDP(final long ts, final Buffer payload)
            throws IllegalArgumentException, IllegalProtocolException {
        return createUdpInternal(ts, payload);
    }

    @Override
    public UDPPacket createUDP(final Buffer payload)
            throws IllegalArgumentException, IllegalProtocolException {
